...
This repository contains a series of templates for creating HTML and PDF documents in R markdown using the style themes employed by NZ Department of Conservation (DOC). Below is a brief guide to getting started with writing and producing tables and figures using R markdown followed by directions to installing the ``DocRR`` package and using it to produce high quality reproducible reports from R.  

###R markdown 

The following is a brief introduction to R markdown, covering some of the basics of creating a ``.Rmd`` document, incorporating R code, writing and producing tables and figures in R markdown. For more detailed documentation download and use the [R markdown cheatsheet](https://www.rstudio.com/wp-content/uploads/2016/03/rmarkdown-cheatsheet-2.0.pdf) and [R markdown reference guide](https://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf). If already familiar with R markdown then skip ahead to the [DocRR](#docrr-package) section to install and begin using the ``DocRR`` package. 
...
R markdown enables this through using R code directly in your text. Code results can be inserted directly into the text of a ``.Rmd`` file by enclosing the code with: 
...
R markdown will automatically produce a bibliography from in-line citations. By default the ``DocRR`` package uses ``bibtex`` citations as the default format. When a template is saved a ``.bib`` file is automatically added to the folder structure. This can be thought of as a reference database much like Endnote or Mendeley, indeed Endnote/Mendeley libraries can usually be exported to ``bibtex``. Each reference cited needs to have an entry in the ``.bib`` file. An easy way to get the ``bibtex`` format for citations is to use [google scholar](https://scholar.google.co.nz/). Search for an article, click on 'Cite' then click on 'BibTeX' (Figure \@ref(fig:googlescholar)). Copy and paste the result into your ``.bib`` file.
...
R Studio supports these templates natively. You can download R Studio [here](https://www.rstudio.com/products/rstudio/download/). 

If using R Studio you can now load pre-made templates for outputting analysis and reporting in HTML or PDF format. To do this go to
...
````
This tells ``pandoc`` that the ``.Rmd`` file is intended for producing a PDF document. If you want to produce a ``.docx`` document you need to remove the ``#`` symbol from ``word_document`` and add ``#`` symbols to every line from ``pdf_document`` and down. So that section of the YAML would now look like this:
